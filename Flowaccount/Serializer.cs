...
namespace Flowaccount
{
    public sealed class Serializer
    {
        ...
        public T StreamDeserialize<T>(Stream target) where T : class
        {
            object _result = Activator.CreateInstance(typeof(T));
            ((StreamResponseModel)_result).Success = true;
            ((StreamResponseModel)_result).Output = target;
            return (T)_result;
        }
		...
		public StringContent ExtractJsonValue<T>(T payload)
		{
			...
			return new StringContent(result.ToString(), Encoding.UTF8, "application/json");
		}
